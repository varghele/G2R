...
class GNN_FULL_CLASS(torch.nn.Module):
    def __init__(self, BATCH_SIZE, NO_MP_ONE, NO_MP_TWO, device):
    ...
    def forward(self,plist):
        ...
        u1=torch.full(size=(self.batch_size, NO_GRAPH_FEATURES_ONE), fill_value=0.1, dtype=torch.float).to(self.device)
        u2=torch.full(size=(self.batch_size, NO_GRAPH_FEATURES_ONE), fill_value=0.1, dtype=torch.float).to(self.device)
        um=torch.full(size=(self.batch_size, NO_GRAPH_FEATURES_ONE), fill_value=0.1, dtype=torch.float).to(self.device)
